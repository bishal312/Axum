...
use axum::{Extension, Json, Router, extract::Query, http::{HeaderMap, StatusCode, header}, response::{IntoResponse, Redirect}, routing::{post,get}};
...
pub async fn login(
    ...
    let user = result.ok_or_else(|| HttpError::bad_request(ErrorMessage::WrongCredentials.to_string()))?;
    ...
    if password_matched {
        ...
        if let Ok(cookie_header) = cookie.to_string().parse() {
            headers.append(header::SET_COOKIE, cookie_header);
        }

        let body = Json(UserLoginResponseDto {
            status: "success".to_string(),
            token,
        });
...
pub async fn verify_email(
    ...
    let user = result.ok_or_else(|| HttpError::unauthorized(ErrorMessage::InvalidToken.to_string()))?;
    ...
    // FIXED: Returning tuple `(headers, response)` directly satisfies `IntoResponse`
...
pub async fn forget_password(
    ...
    let user = result.ok_or_else(|| HttpError::bad_request("Email not found!".to_string()))?;
...
pub async fn reset_password(
    ...
    let user = result.ok_or_else(|| HttpError::bad_request("Invalid or expired token".to_string()))?;
